...
if [ "$CC_SRC_PATH" = "NA" ]; then
  ...
  if [ "$CC_NAME" = "donor" ]; then
    println $'\e[0;32m'donor-asset-transfer$'\e[0m' chaincode
    CC_SRC_PATH="../donor-asset-transfer"
  else
    fatalln "The chaincode name ${CC_NAME} is not supported by this script. Supported chaincode names are: donor, etc.,"
  fi
